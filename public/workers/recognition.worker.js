...
function recognizeSamples(samples) {
  ...
  if (atChest && xRange > 0.1 && yRange > 0.1 && circularMotion(xs2, ys2)) {
...
function completedFistNod(history) {
  let start = 0;
  ...
  const first = samples[0];
  const last = samples.at(-1);
  if (last.time - first.time < 220 || first.open || last.open || ratio(samples, (sample) => sample.open === 0) < 0.8) return false;
  const orientations = samples.map((sample) => {
    const p = sample.hand.landmarks;
    const pitches2 = [5, 9, 13, 17].map((i) => Math.atan2(p[i].z - p[0].z, p[0].y - p[i].y));
    const anchor = pitches2[0];
    const unwrapped = pitches2.map((angle) => anchor + Math.atan2(Math.sin(angle - anchor), Math.cos(angle - anchor))).sort((a, b) => a - b);
  ...
  for (let i = 1; i < orientations.length; i++) {
    ...
    if (Math.abs(wrapped) > 1.2) return false;
    pitches.push(pitches[i - 1] + wrapped);
  }
  const smooth = pitches.map((pitch, i) => [
    pitches[Math.max(0, i - 1)],
    pitch,
    pitches[Math.min(pitches.length - 1, i + 1)]
  ].sort((a, b) => a - b)[1]);
  const excursion = span(smooth);
  if (excursion < 0.35 || span(orientations.map((o) => o.sideways)) > Math.min(0.45, excursion * 0.65) || span(samples.map((sample) => sample.wrist.x)) > 0.25) return false;
  let tail = samples.length - 1;
  while (tail > 0 && last.time - samples[tail].time < 75) tail--;
  if (span(smooth.slice(tail)) > 0.12 || span(samples.slice(tail).map((s) => s.wrist.x)) > 0.035 || span(samples.slice(tail).map((s) => s.wrist.y)) > 0.035) return false;
  return [1, -1].some((direction) => {
    ...
    return outward >= 0.35 && back >= Math.max(0.3, outward * 0.65) && Math.abs(smooth.at(-1) - smooth[0]) <= outward * 0.45 && samples[peak].time - first.time >= 60 && last.time - samples[peak].time >= 60;
  });
}
...
function circularMotion(xs2, ys2) {
  ...
  const width = span(xs2);
  const height = span(ys2);
  const angles = xs2.map((x, index) => Math.atan2((ys2[index] - cy) / height, (x - cx) / width));
  let turn = 0;
  let travel = 0;
...
`));
    ...
    var fi = typeof TextDecoder < "u" ? new TextDecoder("utf-16le") : void 0, Mc = (i, l) => {
      ...
      if (l >>>= 0, p ?? (p = 2147483647), 4 > p) return 0;
      var h = l;
      p = h + p - 4;
...
`))[0] == "Error" && h.shift(), Ai(h);
    ...
    var M = await (async function() {
      ...
      mt ?? (mt = r.locateFile ? r.locateFile ? r.locateFile("ort-wasm-simd-threaded.jsep.wasm", w) : w + "ort-wasm-simd-threaded.jsep.wasm" : new URL("ort-wasm-simd-threaded.jsep.wasm", import.meta.url).href);
      try {
...
function createLandmarkModelLoader(directory, classes) {
  ...
  return function loadModel5() {
    pending2 ?? (pending2 = (async () => {
      ...
      if (!Array.isArray(labels) || labels.length !== classes || labels.some((label) => typeof label !== "string" || !label.trim()) || new Set(labels).size !== classes) {
...
// lib/isl263-runtime.ts
var FRAME_COUNT = 200;
var FEATURES_PER_FRAME = 134;
var MIN_VISIBLE_FRAMES = 16;
var MIN_CONFIDENCE = 0.78;
var MIN_MARGIN = 0.18;
var loadModel2 = createLandmarkModelLoader("/models/isl263-include", 263);
async function recognizeIsl263(sequence) {
...
function hand(frame, handedness) {
...
function interpolateCoordinate(frames2, landmark, axis) {
  ...
  for (let index = 0; index < frames2.length; index += 1) {
    if (frames2[index][landmark][axis] !== 0) continue;
    ...
    frames2[index][landmark][axis] = before.value + (after.value - before.value) * progress;
  }
}
...
var loadModel3 = createLandmarkModelLoader("/models/bsl1064-pose2sign", 1064);
...
var loadModel4 = createLandmarkModelLoader("/models/lse300-swl", 300);
...
// public/models/rsl1000-slovo/labels.json
var labels_default2 = [
  "\u0401",
  "\u0410",
  "\u0420",
...
function personalLanguage({ id: id2, shortName, language, speechLocale }) {
  return {
    id: id2,
    shortName,
    language,
...
var MODEL_ADAPTERS = {
  ...
  rsl: {
    ...
    speechLocale: "ru-RU",
    status: "experimental",
    modelFile: "Official Slovo MViTv2-small-32-2 ONNX",
    automaticVocabularyCount: 1e3,
    vocabulary: labels_default2.slice(0, 1e3),
    inputFormat: "32 unmirrored RGB frames, letterboxed to 224\xD7224, sampled at about 15 fps",
    sequenceLength: 32,
  ...
  ksl_ke: personalLanguage({ id: "ksl_ke", shortName: "KSL \xB7 Kenya", language: "Kenyan Sign Language", speechLocale: "en-KE" }),
