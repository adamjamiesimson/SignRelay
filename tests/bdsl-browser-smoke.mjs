...
// node tests/bdsl-browser-smoke.mjs /path/to/chrome [/path/to/fixture.y4m]
...
try {
  ...
  await writeFile("outputs/bdsl-mobile.png", Buffer.from((await send("Page.captureScreenshot", { format: "png" })).data, "base64"));
  ...
  await writeFile("outputs/bdsl-desktop.png", Buffer.from((await send("Page.captureScreenshot", { format: "png" })).data, "base64"));
  assert(requests.some(request => request.url === `${origin}/models/bdsl401-videomae/model.onnx`), "Expected local pretrained weights request");
